    ...
    for line in Lines:
        ...
        for i , Tok in enumerate(line):
                if Tok == "[":
                    ...
                    expr_end = i 
                if expr_start != 0 and expr_end != 0 :
                    exist_expresie = True
        
        if exist_expresie:

            expresie = line[expr_start : expr_end]
            instructiune = line[: expr_start - 1] #Trebuie sa salvam instructiunea originala 
            AST = Parse(expresie)
            new_Ast = Proces_Expresion(AST)
            #Acum verificam daca ni sa intors un AST sau un rezultat 
            if isinstance(new_Ast , BinOP):
                instructiuni = Expand(new_Ast , instructiune)
            else:
                instructiune.append(str(new_Ast))
                instructiuni.append(instructiune)

            source_code += instructiuni
